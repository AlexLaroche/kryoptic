...
impl MlDsaKey {
    ...
    /// pair, per its own header doc comment. Verified empirically during
    /// this task's implementation to hold for all three parameter sets
    /// (ML-DSA-44/65/87), not just the 65 case the design research probed
    /// live -- see `from_seed_matches_fips204_seed_length_for_all_param_sets`
    /// below.
    pub fn from_seed(
    ...
    /// call site in the integration task (Task 4), not just here.
    ...
    /// working context string, the same pattern used for Phase 3's ECDH
    ...
    fn compute_mu_for_m_prime(
        ...
        let mut parts: Vec<&[u8]> =
            Vec::with_capacity(m_prime_parts.len() + 1);
...
mod tests {
    ...
    /// `sign_prehashed`/`verify_prehashed` (this task's new primitive) to
    /// verify HashML-DSA's `M'` construction and shared `mu` computation
    /// for real, per the coordinator's explicit instruction: this is a
    /// much stronger check than self-consistency alone.
    #[test]
